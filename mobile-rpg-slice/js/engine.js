...
export function updateNoticeView() {
  return {
    ...
    title: "更新公告",
    body: [
      "用語改為水母深域：潮晶、浮游餌、潮息、體階、育成。存檔進度不變。",
...
/** 水母池待命：性格×屬性慢產（全局再 × RANCH_IDLE_GLOBAL_MULT） */
...
export function upgradeFullCostView(state, level) {
  ...
  const feed = upgradePayRow("feed", "浮游餌", snap.feed, state?.feed);
...
export function spineStuckCoachTips(state, { floor, kind = "wipe", failStreak = 0 } = {}) {
  ...
  if (maxLv >= 10 && !earthUnlocked) {
    tips.push(
      "出戰已到 Lv10 軟頂：再升級要礁階石。礁階石通關 1-20、入 2-1 開始掛機出，產量爬到 2-19 滿產，之後章節照掉。而家第一章搵唔到石～"
    );
  } else if (!petN || minLv < expected) {
    tips.push(
      `本關預期大約 Lv${expected}。升級出戰水母（潮露喺「育成 → 練功」掛機攞）可提高效率。`
    );
  }
    ...
    if (minLv < 10) {
      ...
      const skillBit =
        avgSkill < 2
          ...
          : "";
      tips.push(
        `${skillBit}可換戰術／陣型，或者去秘境遇強啲同伴、用珊瑚屑繁殖——唔使等而家未有嘅礁階石。`
      );
    }
  }
...
export function useBagItem(state, itemId, petUid = null) {
  ...
  if (itemId === "ranch_fence") {
    if (state.itemBonus.ranchCap >= RANCH_CAP_BONUS_MAX) {
      return { ok: false, msg: `水母池擴容已達上限（+${RANCH_CAP_BONUS_MAX}）。` };
    }
    state.items[itemId] = have - 1;
    state.itemBonus.ranchCap += 1;
    const cap = ranchCap(state);
    pushLog(state, `用咗欄柵，水母池容量變 ${cap}（永久 +${state.itemBonus.ranchCap}）。`);
    return {
      ok: true,
      msg: `水母池容量 → ${cap}（永久 +${state.itemBonus.ranchCap}/${RANCH_CAP_BONUS_MAX}）`,
      ranchCap: cap,
    };
  }
  ...
  if (itemId === "tide_shift_charm") {
    if (!petUid) return { ok: false, msg: "請先揀一隻水母使用轉屬符。", needsTarget: true };
...
export function useTideShiftCharm(state, uid) {
  ...
  if (have < 1) return { ok: false, msg: "沒有轉屬符。" };
  ...
  pushLog(
    state,
    `【${displayPetName(pet)}】使用轉屬符：${oldEl?.name || oldId} → ${newEl.name}（永久）。`
  );
  return {
    ok: true,
...
function applyOnlineIdleTick(state, elapsed) {
  ...
  if (qiGain > 0.05) gains.push({ id: "qi", name: "潮息", amount: qiGain });
  const feedGain = (state.feed || 0) - feedBefore;
  if (feedGain > 0.05) gains.push({ id: "feed", name: "浮游餌", amount: feedGain });
  const dustGain = (state.dust || 0) - dustBefore;
  if (dustGain > 0.05) gains.push({ id: "dust", name: "螢砂", amount: dustGain });
...
export function claimDaily(state, questId) {
  ...
  if (q.reward.dust) bits.push(`${q.reward.dust} 螢砂`);
  if (q.reward.scrap) bits.push(`${q.reward.scrap} 貝屑`);
...
export function checkAchievements(state) {
  ...
  for (const a of ACHIEVEMENTS) {
    ...
    if (a.reward.dust) bits.push(`${a.reward.dust}螢砂`);
    if (a.reward.scrap) bits.push(`${a.reward.scrap}貝屑`);
...
export function claimBreedGoal(state, goalId) {
  ...
  if (g.reward.dust) bits.push(`${g.reward.dust} 螢砂`);
  if (g.reward.scrap) bits.push(`${g.reward.scrap} 貝屑`);
...
export function soulShopView(state) {
  ...
  return SOUL_SHOP_OFFERS.map((o) => {
    ...
    if (o.grant?.items?.ranch_fence && (bonus.ranchCap | 0) >= RANCH_CAP_BONUS_MAX) {
      capped = true;
      capReason = `水母池加成已達上限（+${RANCH_CAP_BONUS_MAX}）`;
...
/** 心螢核商人購入（afford 心螢核後發放浮游餌／材料／道具） */
export function buySoulShopOffer(state, offerId) {
  ...
  if (offer.grant?.items?.ranch_fence && (bonus.ranchCap | 0) >= RANCH_CAP_BONUS_MAX) {
    return { ok: false, msg: `水母池加成已達上限（+${RANCH_CAP_BONUS_MAX}），無需再換欄柵。` };
  }
  ...
  if (have < offer.cost) {
    return { ok: false, msg: `心螢核不足（需 ${offer.cost}，現有 ${have}）。` };
  }
  ...
  pushLog(state, `心螢核商人購入【${offer.name}】，耗心螢核 ${offer.cost}，獲 ${grantTxt}。`);
  return {
    ok: true,
    msg: `購入 ${offer.name}（耗心螢核 ${offer.cost}）· ${grantTxt}`,
    offerId: offer.id,
    cost: offer.cost,
    grant: offer.grant,
  };
...
export function buyShopOffer(state, offerId) {
  ...
  const cap = ranchCap(state);
  if (owned >= cap) {
    return { ok: false, msg: `水母池已滿（${cap}）。可先放流或升階擴容。` };
  }
  ...
  pushLog(
    state,
    `商肆購入【${displayPetName(pet)}】（${pet.elementName}）直入水母池，耗 ${payCost} 潮晶。`
  );
  checkAchievements(state);
  advanceTutorialCascade(state);
...
export function startDispatch(state, missionId, petUids) {
  ...
  if (uids.length !== mission.needPets) {
    return { ok: false, msg: `需要派出 ${mission.needPets} 隻水母池水母。` };
  }
  const busy = dispatchBusyUids(state);
  ...
  for (const uid of uids) {
    ...
    if (state.pets.some((p) => p.uid === uid)) {
      return { ok: false, msg: "請先將水母撤回水母池再派遣。" };
    }
    const hit = findOwnedPet(state, uid);
    if (!hit || hit.list !== "ranch") return { ok: false, msg: "只能派遣水母池待命水母。" };
...
export function claimDispatch(state, dispatchId, rng = Math.random) {
  ...
  if (scaled?.dust) bits.push(`${scaled.dust}螢砂`);
  if (scaled?.scrap) bits.push(`${scaled.scrap}貝屑`);
  if (scaled?.materials) {
...
/**
 * Soft prestige：漂主後鑄印，重置階段／潮息，保留寵／裝／圖鑑／通關
 */
...
/** 在出戰／水母池中查找水母；回傳 { pet, list, index } */
...
/** 嘗試契約待契約寵物（成功進入水母池）；useFeed 耗浮游餌提升成功率 */
export function tryBondPending(state, encounterId, useFeed = false) {
  ...
  if (roll <= chance) {
    ...
    pushLog(state, `契約成功${feedNote}：${petLabel(pet)} 進入水母池｜技能【${pet.skillName}】。`);
    checkAchievements(state);
    return { ok: true, success: true, msg: `契約成功！${pet.name} 已入水母池` };
  }
...
export function deployPet(state, uid) {
  ...
  if (i < 0) return { ok: false, msg: "水母池中找不到這隻水母。" };
...
export function claimHatch(state, eggUid) {
  ...
  // 蛋入牧場欄；出戰欄唔佔水母池容量（同契約／繁殖／撤回一致）
  const cap = ranchCap(state);
  if (state.ranch.length >= cap) {
    return {
      ok: false,
      msg: `水母池已滿（${state.ranch.length}/${cap}），無法領取。可先清倉放生或出戰。`,
      ranchFull: true,
    };
  }
...
export function undeployPet(state, uid) {
  ...
  const cap = ranchCap(state);
  if (state.ranch.length >= cap) {
    return { ok: false, msg: `水母池已滿（${cap}），無法撤回。` };
  }
  ...
  state.ranch.push(pet);
  pushLog(state, `${pet.name} 撤回水母池。`);
  return { ok: true, msg: `${pet.name} 已回水母池` };
}

export function releasePet(state, uid) {
  if (!state.ranch) state.ranch = [];
  const found = findOwnedPet(state, uid);
  if (!found) return { ok: false, msg: "不在水母欄／水母池。" };
  if (breedBusyUids(state).has(uid)) {
  ...
  pushLog(state, `放生 ${label}，獲心螢核 ${soul}。`);
  return {
    ok: true,
    msg: `放生 ${label}，獲心螢核 ${soul}`,
    soul,
...
export function releasePets(state, uids) {
  ...
  for (const uid of ids) {
    ...
    if (!r.ok) {
      return {
        ok: false,
        msg: count ? `${r.msg}（已放生 ${count} 隻，心螢核 +${totalSoul}）` : r.msg,
        soul: totalSoul,
        count,
        partial: count > 0,
      };
  ...
  return {
    ok: true,
    msg: count === 1 ? names[0] : `放生 ${count} 隻，共獲心螢核 ${totalSoul}`,
    soul: totalSoul,
    count,
  };
}
...
export function dissolveEgg(state, eggUid) {
  ...
  pushLog(state, `退蛋【${label}】，獲心螢核 ${soul}。`);
  return { ok: true, msg: `退蛋 ${label}，心螢核 +${soul}`, soul, egg };
}
...
export function dissolveEggs(state, eggUids) {
  ...
  for (const uid of ids) {
    ...
    if (!r.ok) {
      return {
        ok: false,
        msg: count ? `${r.msg}（已退蛋 ${count} 枚，心螢核 +${total}）` : r.msg,
        soul: total,
        count,
        partial: count > 0,
      };
  ...
  return {
    ok: true,
    msg: count === 1 ? `退蛋 1 枚，心螢核 +${total}` : `退蛋 ${count} 枚，心螢核 +${total}`,
    soul: total,
    count,
  };
}
...
export function upgradePetSkill(state, uid, which = "primary") {
  ...
  if ((state.dust || 0) < cost) return { ok: false, msg: `螢砂不足（需 ${cost}）。` };
...
export function runDungeon(state, dungeonId, opts = {}) {
  ...
  if ((state.tideSeals || 0) > 0) {
    note(
      `印記 ×${state.tideSeals}（全隊攻血 ×${tideSealCombatMult(state.tideSeals).toFixed(2)}）。`
    );
  }
  if (dex.label) {
    note(dex.label);
  ...
  while (round < maxRounds && !ended) {
    ...
    if (won && ended) {
      ...
      if (dailyStoneBonus || dailyScrapBonus || dailyDustBonus || dailyFeedBonus) {
        ...
        if (dailyScrapBonus) bits.push(`+${dailyScrapBonus}貝屑`);
        if (dailyDustBonus) bits.push(`+${dailyDustBonus}螢砂`);
        if (dailyFeedBonus) bits.push(`+${dailyFeedBonus}浮游餌`);
      ...
      if (first && d.firstClearBonus) {
        ...
        note(
          `攻克【${d.name}】，獲潮晶 ${d.reward.stones}、貝屑 ${d.reward.scrap}。首通額外 +${bonusStones} 潮晶／+${bonusScrap} 貝屑！`
        );
        // 首通保底層鑰 1（側枝唔掉層鑰）
        if (!d.isSideBranch && !isBranchDungeonId(dungeonId)) {
        ...
        if (dungeonId === "tide_3") {
          ...
          pushLog(state, "【1-3】已破——融合解鎖；熔潮砂＋2。");
        ...
        note(
          `攻克【${d.name}】，獲潮晶 ${d.reward.stones}、貝屑 ${d.reward.scrap}${streakNote}。`
        );
      }
      ...
      if (challengeMet && challenge?.bonus) {
        ...
        if (challengeScrap) bits.push(`${challengeScrap}貝屑`);
        if (challengeDust) bits.push(`${challengeDust}螢砂`);
      ...
      if (eliteCleared && d.eliteBonus) {
        ...
        note(
          `擊破精英！額外 +${d.eliteBonus.stones || 0} 潮晶${d.eliteBonus.scrap ? `／+${d.eliteBonus.scrap} 貝屑` : ""}。`
        );
      }
      if (bossCleared && d.bossBonus) {
        ...
        note(
          `擊破 BOSS！額外 +${d.bossBonus.stones || 0} 潮晶${d.bossBonus.scrap ? `／+${d.bossBonus.scrap} 貝屑` : ""}。`
        );
      }
      ...
      for (const c of challenges) {
        ...
        if (c.bonus?.scrap) bits.push(`${c.bonus.scrap}貝屑`);
        if (c.bonus?.feed) bits.push(`${c.bonus.feed}浮游餌`);
        if (c.bonus?.dust) bits.push(`${c.bonus.dust}螢砂`);
        if (c.ok && c.bonus) {
          condHits += 1;
      ...
      if (trial) {
        if (trialCheck?.ok && trial.bonus) {
          ...
          note(
            `試煉達成【${trial.label}】→ 分開結算 +${trialStones}石${trialScrap ? `／+${trialScrap}貝屑` : ""}`
          );
        } else {
          note(
...
export function dungeonSweepCost(state, dungeonId, count) {
  ...
  const name = MATERIALS[DUNGEON_ENTRY_MAT_ID]?.name || "潮霧箋";
  return {
    perRun,
    total,
    mats,
...
export function startDungeonSummon(state, dungeonId, count = 1) {
  ...
  if (!cost.canAfford) {
    return {
      ok: false,
      msg: `潮霧箋不足（需 ${cost.total}，現 ${cost.have}）。練功／每日／升階可獲。`,
    };
  }
  if (!spendMaterials(state, cost.mats)) {
    return { ok: false, msg: "潮霧箋不足。" };
  }
  const baseCd = d.cooldownMs || 20_000;
  ...
  const tokenName = MATERIALS[DUNGEON_ENTRY_MAT_ID]?.name || "潮霧箋";
  const msg =
    n > 1
...
export function canDungeonSweep(state, dungeonId, count = null) {
  ...
  if (count != null) {
    ...
    if (!cost.canAfford) {
      return {
        ok: false,
        reason: `潮霧箋不足（需 ${cost.total}，現 ${cost.have}）`,
        cost,
      };
    }
...
export function claimPathQuest(state, questId) {
  ...
  pushLog(state, `出發【${q.name}】達成，獲 ${bits.join("／")}。`);
  return { ok: true, msg: `領取 ${bits.join("／")}` };
}
...
export function tryBreed(state, uidA, uidB, count = 1) {
  if (!uidA || !uidB || uidA === uidB) {
    return { ok: false, msg: "請選擇兩隻不同的水母池水母。" };
  }
  ...
  if (!a || !b) return { ok: false, msg: "雙親必須都在水母池待命。" };
  const dispatchBusy = dispatchBusyUids(state);
...
export function startAbyssDive(state, squadUids, now = Date.now(), opts = {}) {
  ...
  if (ad.freeUsedDate === today) {
    if (!spendMaterials(state, { mist_token: ABYSS_ENTRY_TOKEN_COST })) {
      return { ok: false, msg: `需要潮霧箋 ×${ABYSS_ENTRY_TOKEN_COST}。` };
    }
  ...
  pushLog(state, spentToken ? `踏入深潛（耗潮霧箋×${spentToken}）。` : "今日首潛深潛（免費）。");
