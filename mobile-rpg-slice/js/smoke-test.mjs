...
assert(!uiSrc2.includes("data-challenge-warden"), "ui no warden challenge on train");
...
assert(uiSrc2.includes("第${trunk.frontier}關") || uiSrc2.includes("第${trunk.frontier}關 ·"), "ui spine floor title");
assert(!uiSrc2.includes("掛機層："), "ui no depth row");
assert(!uiSrc2.includes("train-depth-btn"), "ui no depth buttons");
assert(!uiSrc2.includes("今日強化【主脊掛機】"), "ui no daily spot banner on train");
assert(!uiSrc2.includes("data-set-depth"), "ui depth selector removed from train");
assert(!uiSrc2.includes("主脊第"), "ui no redundant 主脊第 line");
assert(!uiSrc2.includes("段主已通"), "ui no warden depth meta on train");
assert(!uiSrc2.includes("掛機 · 潮露"), "ui no idle drip header on train");
...
assert(swSrc.includes("void-tide-pets-v101"), "sw cache bumped");
